...
public class EditProfile : MonoBehaviour
{
    ...
    // Keep these as fallbacks, but we will try to use NavigationManager first
    ...
    public void SetupInputs(UserData data)
    {
        if (data == null) return;
    ...
    IEnumerator LoadExistingProfileImage(string url)
    {
        using (UnityWebRequest request = UnityWebRequestTexture.GetTexture(url))
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                if (currentProfilePicPreview != null) currentProfilePicPreview.texture = CreateRoundedImage(texture);
            }
        }
    }
...
#elif UNITY_ANDROID
        NativeGallery.GetImageFromGallery((path) => { if (!string.IsNullOrEmpty(path)) { profilePicPath = path; StartCoroutine(LoadLocalImage(path)); } });
#endif
    }
    ...
    IEnumerator LoadLocalImage(string path)
    {
        ...
        {
            if (currentProfilePicPreview != null) currentProfilePicPreview.texture = CreateRoundedImage(uploadedPhoto);
        }
        yield return null;
    }
    ...
    Texture2D CreateRoundedImage(Texture2D sourceTexture)
    {
        ...
        for (int y = 0; y < size; y++)
        {
            for (int x = 0; x < size; x++)
            {
                ...
                {
                    roundedTexture.SetPixel(x, y, sourceTexture.GetPixel(offsetX + (int)((float)x / size * minSide), offsetY + (int)((float)y / size * minSide)));
                }
                else roundedTexture.SetPixel(x, y, Color.clear);
            }
        }
    ...
    IEnumerator UpdateUserProfile()
    {
        ...
        if (profileImageData != null) form.AddBinaryData("profile_pic", profileImageData, "profile.png", "image/png");
        ...
        using (UnityWebRequest request = UnityWebRequest.Post(updateProfileApiUrl, form))
        {
            ...
            if (request.result == UnityWebRequest.Result.Success)
            {
                ...
                // 🔥 FIX: Manually close THIS Edit Panel first
                this.gameObject.SetActive(false);

                // 2. Now Trigger Home Button to reset Navigation State
                ...
                {
                    // Fallback if NavigationManager isn't found
                    if (homePanel) homePanel.SetActive(true);
                }
            }
            else
            {
                Debug.LogError($"Failed: {request.responseCode} - {request.downloadHandler.text}");
    ...
    void ResetSaveState() { isSaving = false; if (saveButton) saveButton.interactable = true; }
    void ShowMessage(string text, Color color) { if (messageBox) { messageBox.text = text; messageBox.color = color; messageBox.gameObject.SetActive(true); Invoke(nameof(HideMessage), 4f); } }
    void HideMessage() { if (messageBox) messageBox.gameObject.SetActive(false); }
}